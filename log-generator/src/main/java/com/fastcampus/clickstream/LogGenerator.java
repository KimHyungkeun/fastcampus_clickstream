...
public class LogGenerator implements Runnable{
    ...
    private final static long MINUMIM_SLEEP_TIME = 500;
    ...
    public void run() {
        ...
        while (isDuration(startTime)) {
            long sleepTime = MINUMIM_SLEEP_TIME + Double.valueOf(rand.nextDouble() * (MAXIMUM_SLEEP_TIME - MINUMIM_SLEEP_TIME)).longValue();
            try {
            ...
            String log = String.format("%s %s %s %s %s %s %s", ipAddr, offsetDateTime, method, url, responseCode, responseTime, sessionId);
